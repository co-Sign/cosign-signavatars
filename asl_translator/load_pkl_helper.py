...
class CUDAUnpickler(pickle.Unpickler):
    ...
    def find_class(self, module, name):
        if module == 'torch.cuda' and name == '_rebuild_tensor_v2':
            return lambda *args: torch.Tensor(*args)
...
def load_pkl_file(file_path):
    ...
    # Method 2: Regular torch load with CPU mapping
    try:
    ...
    # Method 3: Binary modification approach
    try:
        with open(file_path, 'rb') as f:
            pickle_data = f.read()
        
        # Replace CUDA references with CPU
        pickle_data = pickle_data.replace(b'torch.cuda', b'torch.cpu')
        pickle_data = re.sub(b'cuda:[0-9]+', b'cpu', pickle_data)
